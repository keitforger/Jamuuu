...
destSlotIndex)
 ...
 Debug.LogError($"Cannot find slots: sourceIndex={sourceSlotIndex}, destIndex={destSlotIndex}");
    ...
    {
        yield return new WaitUntil(() => JamuSystem.Instance != null && JamuSystem.Instance.jamuDatabase != null);
        TampilkanDariInventory();
        FixScrollViewContentVisibility();
        isScrolling = false;
        SetSlotDragEnabled(true);
        ResetSlotCombine();
        if (hasilJamuImage != null) hasilJamuImage.gameObject.SetActive(false);
        if (namaJamuText != null) namaJamuText.gameObject.SetActive(false);
