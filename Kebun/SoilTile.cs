...
public class SoilTile : MonoBehaviour
{
    ...
    void Update()
    {
        // Cek klik/tap pada tile ini via Input System baru
        ...
        // Untuk mobile (opsional): tap pertama pada tile
        if (Touchscreen.current != null && Touchscreen.current.primaryTouch.press.wasPressedThisFrame)
        {
            Vector2 wp = Camera.main.ScreenToWorldPoint(Touchscreen.current.primaryTouch.position.ReadValue());
            Collider2D hit = Physics2D.OverlapPoint(wp);
    ...
    public void Harvest()
    {
        ...
        BahanItem hasil = JamuSystem.Instance.GetBahan(benihItem.producesBahanName);
